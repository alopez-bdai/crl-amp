...
class RolloutStorage:
    class Transition:
        def __init__(self):
            ...
            # todo: properly handle rewards_raw
    ...
    def __init__(self,
        ...
        self.rewards = [torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device) for _ in
                        range(num_values)]
        self.rewards_raw = [torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device) for _ in
                            range(num_values)]
    ...
    def compute_returns(self, last_values, gamma, lam):
        ...
        for i in range(self.num_values):
            ...
            self.advantages[i] = (self.advantages[i] - self.advantages[i].mean()) / (self.advantages[i].std() + 1e-8)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                if self.num_values==1:
                    target_values_batch = target_values_batch[0]
                    returns_batch = returns_batch[0]
